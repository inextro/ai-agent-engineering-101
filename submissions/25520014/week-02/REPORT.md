...
이 오버헤드 때문에 **`tokens`는 이 실험에서 독립 정보를 거의 주지 않는다.** 실측치의 약
97%가 대화와 무관한 고정비이고, 그래서 실측 배수(5.5배)가 `iters` 배수(5.6배)와 거의
같아진다. 보정하면 4.3배로 내려가는데, 그 차이가 ReAct의 호출당 대화량이 더 크다는 뜻이다
(전문을 계속 이어붙이므로 당연하다). 하네스 안에 하네스를 넣은 대가이며, raw Messages
API로 부르면 이 항은 사라진다.
...
이 태스크에서는 ReAct가 네 지표 전부에서 이겼다. 성공률 3/3 대 0/3, 반복 횟수 2.33 대
13.0(5.6배), 보정 토큰 2,793 대 12,088(4.3배), 시간 13.4초 대 93.2초(7.0배)이고,
...
났다. 세 런이 13 iters로 완전히 같은 것은 우연한 실패가 아니라 구조가 만든 실패라는
증거다. 결정적인 대목은 그 다음이다. 단계가 `OFF_PLAN`으로 끝나면 그 단계에서 이미 얻은
